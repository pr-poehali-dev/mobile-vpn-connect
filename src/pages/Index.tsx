...
type Tab = "connect" | "servers" | "history" | "settings";
            ...
            <div className="animate-fade-in-up space-y-5">
              ...
              <div>
                ...
                <div className="rounded-xl border border-white/[0.05] bg-white/[0.02] divide-y divide-white/[0.04]">
                  {[
                    { label: "Kill Switch", sub: "Блокировать трафик при разрыве VPN", value: killSwitch, set: setKillSwitch, icon: "ShieldOff" },
                    { label: "Защита от DNS-утечек", sub: "Использовать зашифрованный DNS", value: dnsLeak, set: setDnsLeak, icon: "Eye" },
                    { label: "Автоподключение", sub: "Подключаться при запуске приложения", value: autoConnect, set: setAutoConnect, icon: "Wifi" },
                    { label: "Split Tunneling", sub: "Выбрать приложения для VPN", value: splitTunnel, set: setSplitTunnel, icon: "GitBranch" },
                  ].map((item) => (
                    <div key={item.label} className="flex items-center justify-between px-4 py-3">
                      ...
                      >
                        <div className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-white transition-all duration-300 shadow-sm ${
